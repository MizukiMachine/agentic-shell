...
func GenerateMissingSkills(env *Envelope) error {
	...
	for _, skill := range env.Match.MissingSkills {
		filename := filepath.Join(".claude", "skills", slugify(skill.Name), "SKILL.md")
		result.Files = append(result.Files, GeneratedFile{
			Path:    filename,
			Content: renderSkillPlaceholder(skill),
		})
	}
...
func WriteGeneratedFiles(env *Envelope, baseDir string, overwrite bool) error {
...
func renderSkillPlaceholder(skill SkillRequirement) string {
	return strings.TrimSpace(fmt.Sprintf(`
---
name: "%s"
description: "Placeholder skill generated for %s"
---
